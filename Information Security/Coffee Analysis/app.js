...
(function() {
  ...
  getFrequencies = function(text) {
    var ch, charArray, chars, charsCount, i, _ref;
    i = 0;
    chars = {};
    charArray = [];
    charsCount = 0;
    while (i <= text.length) {
      if (('9' >= (_ref = text[i]) && _ref >= '0')) {
        ch = text.slice(i, +(++i) + 1 || 9e9);
  ...
  printTable = function(charFreq, alphabetFreq) {
    var body, decFreqRow, decRow, encFreqRow, encRow, i, key, table, _i, _ref;
    key = {};
    body = document.body;
    ...
    for (i = _i = 0, _ref = charFreq.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
  ...
  printText = function(text, key) {
    var ch, div, hr, i, res, _ref;
    res = '';
    i = 0;
    while (i <= text.length) {
      if (('9' >= (_ref = text[i]) && _ref >= '0')) {
  ...
  printKey = function(key) {
    var body, ch, decRow, encRow, hr, table;
    hr = document.createElement('hr');
    body = document.body;
    table = document.createElement('table');
    table.classList.add('table');
    ...
    body.appendChild(table);
    return body.appendChild(hr);
  };

  key = printTable(getFrequencies(text), alphabetFreq);

  key.decodeChar = function(ch, value) {
    var i;
    for (i in this) {
      if (key[i] === value) {
        key[i] = '{' + i + '}';
